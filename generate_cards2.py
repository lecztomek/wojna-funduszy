...
def draw_text(draw: ImageDraw.ImageDraw, spec: Dict[str, Any], value: str):
    x, y = spec["xy"]
    max_width = int(spec.get("max_width", 99999))
    font = load_font(spec.get("font", "DejaVuSans.ttf"), int(spec.get("size", 28)))
    color = hex_to_rgb(spec.get("color", "#FFFFFF"))
    stroke = int(spec.get("stroke", 0))
    stroke_color = hex_to_rgb(spec.get("stroke_color", "#000000"))
